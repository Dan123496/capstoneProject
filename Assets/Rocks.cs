...
public class Rocks : MonoBehaviour
{
    ...
    void Start()
    {
        // sets the rate at which the rocks will fall, using fixDeltaTime not deltatime so the speed is not effected by frame rate.
    ...
    void CheckCollision()
    {
        //calls the raycast hit 
        RaycastHit2D hit;
        // uses the raycasts ThrowRays function to check for collisions with the rock. 
        // this is done in all directions as the rock could be hit in any directions. 
        ...
        // if the rock collides with player 1 from any direction ThrowRays will return 1
        // so if any of the weAreColliding are equal to 1, this rock has collided with player1 
        if (weAreColliding == 1 || weAreColliding2 == 1 || weAreColliding3 == 1 || weAreColliding4 == 1)
        {
            // this.gameObject refers to this rock, the rock that collided.
            // setActive(false), disables this rock so it can no longer interact with anything in game
        ...
        // if the rock collides with player 2 from any direction ThrowRays will return 2
        // so if any of the weAreColliding are equal to 2, this rock has collided with player2 
        else if (weAreColliding == 2 || weAreColliding2 == 2 || weAreColliding3 == 2 || weAreColliding4 == 2)
        {
            ...
            Debug.Log("player2 score = " + roof.GetComponent<creatFallingObjects>().player2Score);
        }
